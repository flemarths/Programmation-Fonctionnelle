...
let rec fives n = List.map (function x->5*x) (range 1 (n/5)) ;;

let rec makeint c = List.filter (function x -> x>0 ) (List.map (function x-> int_of_string(x)) (String.split_on_char ' ' c));;
